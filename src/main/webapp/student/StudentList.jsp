    ...
    <div class="table-container">
        <div class="table-header">
            ...
            <div class="table-actions">
                <form action="${pageContext.request.contextPath}/student/list" method="get" class="search-form">
                    ...
                    <a href="${pageContext.request.contextPath}/student/list" class="btn btn-default btn-sm">清除</a>
                    <% } %>
                </form>
                <a href="${pageContext.request.contextPath}/student/add" class="btn btn-success">新增学生</a>
                <a href="${pageContext.request.contextPath}/export?type=student" class="btn btn-info">导出CSV</a>
            </div>
        </div>
        <table>
            <thead>
            ...
            %>
                <tr>
                    <td><%= s.getId() %></td>
                    <td><%= s.getStudentNo() != null ? s.getStudentNo() : "" %></td>
                    <td><%= s.getName() %></td>
                    <td><%= s.getGender() != null ? s.getGender() : "男" %></td>
                    <td><%= s.getAge() %></td>
                    <td><%= s.getStatus() != null ? s.getStatus() : "在读" %></td>
                    <td>
                        ...
                        <form action="${pageContext.request.contextPath}/student/delete" method="post" style="display:inline;" onsubmit="return confirm('确定删除该学生吗？')">
            ...
            %>
                <tr>
                    <td colspan="7" style="text-align:center; padding:40px; color:var(--ink-pale);">暂无学生数据</td>
                </tr>
            <%
                }
            %>
        ...
        <div class="pagination">
            <%
                ...
                if (currentPage > 1) {
            %>
                <a href="<%= ctx %>/student/list?page=1<%= searchParam %>">首页</a>
                <a href="<%= ctx %>/student/list?page=<%= currentPage - 1 %><%= searchParam %>">上一页</a>
            <%
                } else {
            %>
            ...
            %>
                <a href="<%= ctx %>/student/list?page=<%= i %><%= searchParam %>"><%= i %></a>
            <%
                    }
                }
            ...
            %>
                <a href="<%= ctx %>/student/list?page=<%= currentPage + 1 %><%= searchParam %>">下一页</a>
                <a href="<%= ctx %>/student/list?page=<%= totalPages %><%= searchParam %>">末页</a>
            <%
                } else {
            %>
