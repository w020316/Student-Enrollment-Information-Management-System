    ...
    <div class="table-container">
        <div class="table-header">
            ...
            <div class="table-actions">
                <form action="${pageContext.request.contextPath}/course/list" method="get" class="search-form">
                    <input type="text" name="name" placeholder="搜索课程名称..." value="<%= request.getAttribute("keyword") != null ? request.getAttribute("keyword") : "" %>" class="search-input">
                    <button type="submit" class="btn btn-info btn-sm">搜索</button>
                    <% String keyword = (String) request.getAttribute("keyword"); %>
                    <% if (keyword != null && !keyword.isEmpty()) { %>
                    <a href="${pageContext.request.contextPath}/course/list" class="btn btn-default btn-sm">清除</a>
                    <% } %>
                </form>
                <a href="${pageContext.request.contextPath}/course/add" class="btn btn-success">新增课程</a>
                <a href="${pageContext.request.contextPath}/export?type=course" class="btn btn-info">导出CSV</a>
            </div>
        </div>
        <table>
            <thead>
            ...
            %>
                <tr>
                    <td><%= c.getId() %></td>
                    <td><%= c.getCourseNo() != null ? c.getCourseNo() : "" %></td>
                    <td><%= c.getName() %></td>
                    <td><%= c.getCredit() %></td>
                    <td><%= c.getHours() %></td>
                    <td>
                        <a href="${pageContext.request.contextPath}/course/view?id=<%= c.getId() %>" class="btn btn-info btn-sm">查看</a>
                        <a href="${pageContext.request.contextPath}/course/update?id=<%= c.getId() %>" class="btn btn-warning btn-sm">编辑</a>
                        <form action="${pageContext.request.contextPath}/course/delete" method="post" style="display:inline;" onsubmit="return confirm('确定删除该课程吗？')">
            ...
            %>
                <tr>
                    <td colspan="6" style="text-align:center; padding:40px; color:var(--ink-pale);">暂无课程数据</td>
                </tr>
            <%
                }
            %>
        ...
        <div class="pagination">
            <%
                ...
                if (currentPage > 1) {
            %>
                <a href="<%= ctx %>/course/list?page=1<%= searchParam %>">首页</a>
                <a href="<%= ctx %>/course/list?page=<%= currentPage - 1 %><%= searchParam %>">上一页</a>
            <%
                } else {
            %>
            ...
            %>
                <a href="<%= ctx %>/course/list?page=<%= i %><%= searchParam %>"><%= i %></a>
            <%
                    }
                }
            ...
            %>
                <a href="<%= ctx %>/course/list?page=<%= currentPage + 1 %><%= searchParam %>">下一页</a>
                <a href="<%= ctx %>/course/list?page=<%= totalPages %><%= searchParam %>">末页</a>
            <%
                } else {
            %>
